...
import XProgressBar from '../xProgressBar.ts';
        
const testXProgressBar = function(sliderSettings) {
         
    describe('XProgressBar', function() {
        ...
        it('Должен быть объявлен', function() {
            expect(XProgressBar).toBeDefined();
        });

        it('Метод render вставляет div с классом "progressBar x-progressBar" перед элементом, переданном в параметрах', function() {

            view.state.progressBars[0].progressBarEl.remove();
            view.state.progressBars[0].render(view.state.runners[0].runnerEl);

            expect(view.state.progressBars[0].progressBarEl.className).toBe('progress-bar x-progress-bar');
            expect(view.state.runners[0].runnerEl.previousElementSibling).toBe(view.state.progressBars[0].progressBarEl);

        });
        
        it('Метод setCurrent устанавливает начальную ширину прогресс бара', function() {

            view.state.progressBars[0].setCurrent(view.sliderSettings.current, view.sliderSettings.start, view.sliderSettings.step);

            const expectedValue = Math.round((view.sliderSettings.current - view.sliderSettings.start) / view.sliderSettings.step);

            expect(view.state.progressBars[0].progressBarEl.style.width).toBe(expectedValue + 'em');
            
        });


        it('Метод countProgressBarSize устанавливает ширину прогресс бара при наступлении события и возвращает это значение', function() {

            for (let i = view.state.scale.scaleEl.getBoundingClientRect().left ; i < view.state.scale.scaleEl.getBoundingClientRect().right; i++ ) {
...
export default testXProgressBar;