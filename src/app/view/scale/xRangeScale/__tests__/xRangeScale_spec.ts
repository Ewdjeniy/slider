...
const testXRangeScale = function(sliderSettings) {
    ...
    describe('XRangeScale', function() {    
        ...
        it('Метод returnScaleStart возвращает координату начала шкалы сложенную с шириной левой границы и левого внутреннего отступа по оси X', function() {
            ...
            const startScaleLeftPosition = view.state.scale.scaleEl.getBoundingClientRect().left + parseFloat(getComputedStyle(view.state.scale.scaleEl).marginLeft) + parseFloat(getComputedStyle(view.state.scale.scaleEl).borderLeft) + parseFloat(getComputedStyle(view.state.scale.scaleEl).paddingLeft);
...
export default testXRangeScale;