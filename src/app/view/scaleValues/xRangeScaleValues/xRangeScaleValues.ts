    ...
    setScaleValues(scaleValuesAmount, start, end, step, decimalPlaces, progressBarFontSize): void {
        if (scaleValuesAmount == 1) {
            ...
            for (let i = 1; i < scaleValuesAmount - 1; i++) {
                ...
                console.log(stepsAmountInOneValue * i);
            ...
            for (let i = 0; i < mediumValues.length; i++) {
                mediumValues[i].style.left = parseFloat(mediumValues[i].style.left) - parseFloat(getComputedStyle(mediumValues[i]).width) / 3 + 'px';
            }
        }
    }
    
    render(scaleEl, scaleValues): void {
        this.scaleValuesEl.className = scaleValues ? 'sсale-values x-range-scale-values x-sсale-values_on' : 'sсale-values x-range-scale-values'; 
