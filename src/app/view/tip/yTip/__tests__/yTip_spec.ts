...
import YTip from '../yTip.ts';
        
const testYTip = function(sliderSettings) {
    
    describe('YTip', function() {
        ...
        it('Метод render вставляет div с классом "tip y-tip" в элемент, переданный в параметрах', function() {

            view.state.tips[0].tipEl.remove();
            view.state.tips[0].render(view.state.runners[0].runnerEl);

            expect(view.state.tips[0].tipEl.className).toBe('tip y-tip');
        ...
        it('Метод showTip переписывает значение элемента в зависимости от высоты прогресс бара и возвращает это значение', function() {
